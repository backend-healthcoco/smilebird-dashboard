...
//						doctorObjectId, locationObjectId, hospitalObjectId);
	...
	public RegisteredPatientDetails registerNewPatient(PatientRegistrationV3Request request) {
		...
		try {
			...
			userCollection.setCreatedFrom("ADMIN");
			userCollection = userRepository.save(userCollection);
...
//			PatientCollection patientCollection = new PatientCollection();
			...
			if (request.getSmileBuddyId() != null) {
				...
				if (smileBuddyUserCollection != null)
					registeredPatientDetails.setSmileBuddyName(smileBuddyUserCollection.getFirstName());
			}
			...
			sendWelcomeMessageToPatient(patientCollection, request.getMobileNumber());
